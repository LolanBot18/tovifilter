    ...
    if 2 < len(message.text) < 100:    
        ...
        if len(btn) > 10: 
            ...
            if poster:
                ...
                await message.reply_text(f"<b>requested from : <b>{}<\b> 👇നിങ്ങൾ ചോദിച്ച മൂവി ലഭിക്കാൻ താഴെ കാണുന്ന ബട്ടണിൽ ക്ലിക്ക് ചെയ്യുക😇 {search} ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
        ...
        if API_KEY:
            poster=await get_poster(search)
        if poster:
            await message.reply_photo(photo=poster, caption=(f"f"**🗂️ Title:** {search}\n**⭐ Rating:** {random.choice(RATING)}\n**🎭 Genre:** {random.choice(GENRES)}\n** © Uploaded by {message.chat.title}**")
        ...
        if files:
            for file in files:
                ...
                btn.append([
                     ...
                     InlineKeyboardButton(text=f"📂 {get_size(file.file_size)}", url=f"https://telegram.dog/{nyva}?start=subinps_-_-_-_{file_id}")
                ])
        else:
            return
        if not btn:
